...
const SearchBar = ({ faqs, onSearch, onFilterChange, onAIResponse }) => {
  ...
  useEffect(() => {
    ...
    const uniqueCategories = ['all', ...new Set(faqs.map(faq => faq.category).filter(Boolean))];
  ...
  const handleAskAI = async () => {
    ...
    try {
      ...
      const topFAQ = rankedFAQs.length > 0 ? rankedFAQs[0] : null;
      
      // Get AI response with context from top FAQs
      const context = topFAQ ? {
      ...
      const additionalContext = rankedFAQs.slice(0, 3).map(faq => ({
